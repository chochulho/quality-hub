...
interface Props {
  /** SOURCE_DOMAIN_CONFIG key, e.g. 'auditsay'. Defaults to 'quality-hub'. */
  sourceDomain?: string
  /** Pre-selected tool key (overrides domain config default) */
  toolKey?: string
}

export default function RegisterForm({ sourceDomain = 'quality-hub', toolKey }: Props) {
    ...
    try {
      const res = await fetch('/api/signup', {
        ...
        body: JSON.stringify({
          ...
          password,
          name,
          orgName: memberType === 'corporate' ? orgName : undefined,
          orgType: memberType,
          sourceDomain,
          toolKey,
        }),
      ...
      // If email is pre-confirmed (external SSO domains), sign in immediately
      if (data.skipEmailConfirm) {
        const supabase = createClient()
        const { error: signInError } = await supabase.auth.signInWithPassword({ email, password })
        if (signInError) {
          // Still registered; ask user to log in manually
          setSuccessState({ type: 'done' })
        } else {
      ...
      if (data.requiresApproval) {
        setSuccessState({ type: 'pending_approval' })
      } else {
        setSuccessState({ type: 'email_confirm' })
      }
    } catch {
  ...
  if (successState) {
    ...
    if (successState.type === 'pending_approval') {
      return (
        <div className="bg-white border border-border rounded-3xl p-8 shadow-sm text-center">
          {icon}
          <h2 className="text-xl font-extrabold text-brand-navy mb-2">가입 신청 완료!</h2>
          <p className="text-sm text-muted-foreground leading-relaxed mb-6">
            기업 회원 신청이 접수되었습니다.<br />
            관리자 검토 후 이메일로 활성화 안내를 드립니다.<br />
          ...
          >
            로그인하러 가기 <ArrowRight className="h-4 w-4" />
          </button>
        </div>
      )
    }

    if (successState.type === 'email_confirm') {
      return (
        <div className="bg-white border border-border rounded-3xl p-8 shadow-sm text-center">
          {icon}
          <h2 className="text-xl font-extrabold text-brand-navy mb-2">이메일을 확인해주세요</h2>
          <p className="text-sm text-muted-foreground leading-relaxed mb-6">
            <span className="font-semibold text-foreground">{email}</span>로<br />
            가입 확인 이메일을 발송했습니다.<br />
            링크를 클릭하면 바로 이용하실 수 있습니다.
          </p>
          <button
            onClick={() => router.push('/login')}
            className="inline-flex items-center gap-2 bg-brand-orange text-white rounded-full px-6 py-3 font-semibold hover:bg-brand-orange-hover transition-all text-sm"
    ...
    // type === 'done' (external, auto sign-in failed)
    return (
      <div className="bg-white border border-border rounded-3xl p-8 shadow-sm text-center">
        {icon}
        <h2 className="text-xl font-extrabold text-brand-navy mb-2">가입 완료!</h2>
        <p className="text-sm text-muted-foreground leading-relaxed mb-6">
          계정이 생성되었습니다. 로그인 후 이용해주세요.
        </p>
        <button
          onClick={() => router.push('/login')}
          className="inline-flex items-center gap-2 bg-brand-orange text-white rounded-full px-6 py-3 font-semibold hover:bg-brand-orange-hover transition-all text-sm"
    ...
    <div className="bg-white border border-border rounded-3xl p-8 shadow-sm">
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="rounded-xl bg-muted/60 border border-border px-4 py-3 text-xs text-muted-foreground">
          {isExternal
            ? '스타터 플랜으로 시작합니다. 선택한 도구를 즉시 이용할 수 있습니다.'
