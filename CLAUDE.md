...
## 작업 원칙
- 큰 변경(라우트 구조, DB 스키마, 의존성)은 반드시 계획을 먼저 제시하고 사용자 승인 후 진행한다.
- 한 세션에서 SPEC-WORKSPACE.md §14의 한 항목만 처리한다. 여러 항목을 묶지 않는다.
- DB 마이그레이션, npm install, 환경변수 변경 등 시스템 영향이 있는 명령은 사용자 확인 없이 실행하지 않는다.
- 작업 완료 후 변경 범위(git diff 요약)를 보고한다.
...
- SPEC-WORKSPACE.md (v2): 워크스페이스 / 요금제 / 도구 3분류 / FMEA 전략 (CLAUDE.md·SPEC.md와 충돌 시 우선)