...
import DocHierarchyView from "@/components/qms/DocHierarchyView"
import { ArrowLeft, PenLine, Eye, Download, LayoutList, GitBranch } from "lucide-react"
        ...
        <div className="flex items-center gap-2">
          <div className="flex rounded-xl border border-border overflow-hidden text-sm">
            <button type="button" onClick={() => setViewMode('list')}
              className={`inline-flex items-center gap-1.5 px-3 py-2 transition-colors ${viewMode === 'list' ? 'bg-brand-navy text-white' : 'text-muted-foreground hover:bg-muted/40'}`}>
              <LayoutList className="h-3.5 w-3.5" />목록
            </button>
            <button type="button" onClick={() => setViewMode('hierarchy')}
              className={`inline-flex items-center gap-1.5 px-3 py-2 transition-colors ${viewMode === 'hierarchy' ? 'bg-brand-navy text-white' : 'text-muted-foreground hover:bg-muted/40'}`}>
              <GitBranch className="h-3.5 w-3.5" />체계
            </button>
          </div>
        </div>
        <Link href="/qms-wizard"
